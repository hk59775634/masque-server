...
// HTTP Datagrams (RFC 9297): RFC 9484 Context ID (varint) is peeled when present; unknown non-zero IDs drop unless CONNECT_IP_STUB_ECHO_CONTEXTS allowlist (dev); inner IPv4/IPv6 is ACL-checked then echoed unless CONNECT_IP_UDP_RELAY (IPv4/UDP) or CONNECT_IP_ICMP_RELAY (IPv4 ICMP Echo) handles the packet (user-space relay + reply), or CONNECT_IP_TUN_FORWARD (Linux) writes allowed IP payloads to a per-session TUN. Optional CONNECT_IP_TUN_LINK_UP runs ip link set up after TUN open. Opaque inner payloads are echoed. SNAT is not configured in-process.
